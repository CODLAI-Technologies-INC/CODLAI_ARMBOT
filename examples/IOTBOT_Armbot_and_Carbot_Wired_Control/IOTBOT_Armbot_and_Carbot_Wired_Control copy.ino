...
void showCarbotScreen()
{
  ...
  lcdWriteFixedTxt(17, 1, "---", 3);                     // placeholder; will be replaced immediately
                                                        // Distance instead of Steer/Drive
  lcdWriteFixedTxt(0, 2, "Mesafe: --- cm  ", 16);
...
void loop()
{
  ...
  // Ultrasonic sensor logic for CARBOT
  static unsigned long lastDistReadMs = 0;
  bool anyButtonPressed = b1Now || iotbot.button2Read() || iotbot.button3Read();

  if (anyButtonPressed)
  {
    if (carBot.isUltrasonicActive())
    {
      carBot.disableUltrasonic();
      // Restore LED state which ultrasonic disabled
      carBot.controlLED(ledState);
    }
  }
  else
  {
    ...
    if (now - lastDistReadMs > 300)
    {
      float dist = carBot.readUltrasonicCM();
      if (dist >= 0)
      {
        char buf[17];
        snprintf(buf, sizeof(buf), "Mesafe: %3d cm  ", (int)dist);
        lcdWriteFixedTxt(0, 2, buf, 16);
      }
      lastDistReadMs = now;
    }
  }
